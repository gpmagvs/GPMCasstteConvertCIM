...
namespace GPMCasstteConvertCIM.CasstteConverter
{
    public partial class clsConverterPort : IModbusHSable, INotifyPropertyChanged
    {
        ...
        private bool _PartsReplacing = false;
        ...
        public bool Maintaining
        {
            ...
            set
            {
                if (_Maintaining != value)
                {
                    _Maintaining = value;
                    PropertyChanged?.Invoke(this, new PropertyChangedEventArgs("Maintaining"));

                }
            }
        }
        public bool PartsReplacing
        {
            get => _PartsReplacing;
            set
            {
        ...
        public DateTime CarrierInstallTime { get; protected set; } = DateTime.Now;
            ...
            if (Properties.IsConverter && Properties.ModifyAGVSCargoIDWithWebAPI && CallRemoveRackCarrierIDAPI)
            {
                _ = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        await AGVSiniRead.ReadAGVSini(Properties.NameInAGVS, 1);
                        (bool confirm, string response, string errorMsg) result = await API.KGAGVS.RackStatusAPI.DeleteCSTID(Properties.NameInAGVS, 1, Properties.NameInAGVS);
                    ...
                    if (value)
                    {
                        ...
                        if (Properties.SecsReport)
                        {
                            Task.Factory.StartNew(async () =>
                            {
                                ...
                                await CarrierWaitoutSecsGemReportProcess();
                            });
                        }
